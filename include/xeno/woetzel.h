...
#define IO_ATTR(m)          m(const_cast<IO&>(io).io_attr(#m,*(&m)))
#define IO_ATTR_NUL(m)      m(const_cast<IO&>(io).io_attr_nul(#m,*(&m)))
#define IO_ATTR_DEF(m,val)  m(const_cast<IO&>(io).io_attr_def(#m,*(&m),val))
#define IO_ATTR_OPT(m)      m(const_cast<IO&>(io).io_attr_def(#m,*(&m),*(&m)))
// For IO_ENUM you can make a specialisation of the io_enum_traits template above
#define IO_ENUM(m)          m(const_cast<IO&>(io).io_enum(#m,*(&m)))
#define IO_ENUM_TEXT(m)     m(const_cast<IO&>(io).io_enum_text(#m,*(&m)))
#define IO_TEXT(m)          m(const_cast<IO&>(io).io_text(#m,*(&m)))
#define IO_INNER(m)         m(const_cast<IO&>(io).io_text(*(&m)))
#define IO_INNER_NUL(m)     m(const_cast<IO&>(io).io_text_nul(*(&m)))
#define IO_INNER_DEF(m,val) m(const_cast<IO&>(io).io_text_def(*(&m),val))
#define IO_LINK(m)          m(const_cast<IO&>(io).io_link(#m,*(&m)))
#define IO_PART(m)          m(const_cast<IO&>(io).io_part(#m))
#define IO_LIST(e,c)        c(std::move(const_cast<IO&>(io).io_list(#e,#c,c)))
#define IO_ANON_LIST(c)  	c(std::move(const_cast<IO&>(io).io_list(#c,nullptr,c)))

#define IO_INIT {

#define IO_ARRAY(e,c)       const_cast<IO&>(io).io_list(#e, #c, &c[0], c+(sizeof(c)/sizeof(c[0])));
